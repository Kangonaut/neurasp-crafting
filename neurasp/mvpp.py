...
class MVPP(object):
    ...
    def parse(self, program):
        ...
        elif type(program) is str and re.sub(
            r"\n%[^\n]*", "\n", program
        ).strip().endswith((".", "]")):
            lines = program.split("\n")
        else:
    ...
    def normalize_probs(self):
        for ruleIdx, list_of_bools in enumerate(self.learnable):
            ...
            for atomIdx, b in enumerate(list_of_bools):
                if b == True:
                    self.parameters[ruleIdx][atomIdx] = (
                        self.parameters[ruleIdx][atomIdx]
                        / summation
                ...
                for modelIdx, model in enumerate(models):
                    ...
                    if cEqualsVi in model:
                        if self.parameters[ruleIdx][i] != 0:
                            ...
                            numerator += probs[modelIdx] / (
                                self.parameters[ruleIdx][i] + self.eps
                            )
    ...
    def gradients_multi_obs(self, list_of_obs):
        ...
        for obs in list_of_obs:
            gradients = [
                [c + d for c, d in zip(i, j)]
                for i, j in zip(gradients, self.gradients_one_obs(obs))
            ]
        return gradients
    ...
    def learn_exact(self, list_of_obs, lr=0.01, thres=0.0001, max_iter=None):
        ...
        print(
            "Start learning by exact computation with {} observations...\n\nInitial parameters: {}".format(
        ...
        while check_continue:
            ...
            dif = [
                [lr * grad for grad in l] for l in self.gradients_multi_obs(list_of_obs)
            ]
            ...
            self.parameters = [
                [c + d for c, d in zip(i, j)] for i, j in zip(dif, self.parameters)
            ]
            self.normalize_probs()

            # we termintate if the change of the parameters is lower than thres
            dif = [
                [abs(c - d) for c, d in zip(i, j)]
                for i, j in zip(old_parameters, self.parameters)
            ]
            print(
                "After {} seconds of training (in total)".format(
                    time.time() - time_init
                )
            )
    ...
    def k_sample(self):
        ...
        result = clingo_control.solve(
            on_model=lambda model: models.append(model.symbols(shown=True))
        )
        models = [[str(atom) for atom in model] for model in models]
        return models
    ...
    def sample_obs(self, obs, num=50):
        ...
        while count < num:
            ...
            clingo_control.ground([("base", [])])
            result = clingo_control.solve(
                on_model=lambda model: models_tmp.append(model.symbols(shown=True))
            )
            if str(result) == "SAT":
    ...
    def sample_obs2(self, obs, num=50):
        ...
        clingo_control.ground([("base", [])])
        clingo_control.solve(
            None, lambda model: candidate_sm.append(model.symbols(shown=True))
        )
    ...
    def learn_by_sampling(
        ...
        print(
            "Start learning by sampling with {} observations...\n\nInitial parameters: {}".format(
                len(list_of_obs), self.parameters
            )
        )
        time_init = time.time()
        ...
        while check_continue:
            ...
            print(
                "Generating {} stable model(s) for each observation...\n".format(
                    num_of_samples
                )
            )
            dif = lr * self.gradients_multi_obs_by_sampling(
                list_of_obs, num=num_of_samples
            )
