...
# repo: https://github.com/azreasoners/NeurASP
# paper: https://doi.org/10.24963/ijcai.2020/243
...
class NeurASP(object):
    def __init__(self, dprogram, nnMapping, optimizers, gpu=False):
        ...
        self.n = (
            {}
        )  # the mapping from nn name to an integer n denoting the domain size; n would be 1 or N (>=3); note that n=2 in theorey is implemented as n=1
        self.e = {}  # the mapping from nn name to an integer e
        self.domain = (
            {}
        )  # the mapping from nn name to the domain of the predicate in that nn atom
        self.normalProbs = None  # record the probabilities from normal prob rules
        self.nnOutputs = {}
        self.nnGradients = {}
        if gpu == True:
            self.nnMapping = {
                key: nn.DataParallel(nnMapping[key].to(self.device))
                for key in nnMapping
            }
        else:
        ...
        self.yoloInfo = (
            []
        )  # (beta version; used when e="yolo") a list for yolo neural network to make prediction, each element is of the form [m, t, domain]
        self.mvpp["program"], self.mvpp["program_pr"], self.mvpp["program_asp"] = (
            self.parse(obs="")
        )
        self.stableModels = (
            []
        )  # a list of stable models, where each stable model is a list
    ...
    def nnAtom2MVPPrules(self, nnAtom):
        ...
        e, t = out.group(2).split(
            ",", 1
        )  # in case t is of the form t1,...,tk, we only split on the first comma
        ...
        # if the neural network is a classification network that outputs probabilities only
            ...
            if n == 1:
                for i in range(e):
                    rule = "@0.0 {}({}, {}, {}); @0.0 {}({}, {}, {}).".format(
                        m, i, t, domain[0], m, i, t, domain[1]
                    )
                    prob = [tuple((m, i, t, 0))]
                    atoms = [
                        "{}({}, {}, {})".format(m, i, t, domain[0]),
                        "{}({}, {}, {})".format(m, i, t, domain[1]),
                    ]
                ...
                print(
                    "Error: the number of element in the domain %s is less than 2"
                    % domain
                )
        return mvppRules
    ...
    def infer(self, dataDic, obs="", mvpp="", postProcessing=None):
        ...
        for m in self.nnOutputs:
            ...
            for t in self.nnOutputs[m]:
                ...
                if self.e[m] == "yolo":
                    if postProcessing:
                        self.nnOutputs[m][t] = postProcessing(
                            self.nnMapping[m](dataTensor)
                        )
                        # Step 2: turn yolo outputs into a set of MVPP probabilistic rules
                        for idx, (label, x1, y1, x2, y2, conf) in enumerate(
                            self.nnOutputs[m][t]
                        ):
                            ...
                            if label in self.domain[m]:
                                ...
                                mvppRules += "@{} {}; @{} -{}.\n".format(
                                    conf, atom, 1 - conf, atom
                                )
                    else:
                        print(
                            "Error: the function for postProcessing yolo output is not specified."
                        )
                        sys.exit()
                else:
                    self.nnOutputs[m][t] = (
                        self.nnMapping[m](dataTensor).view(-1).tolist()
                    )
        ...
        for ruleIdx in range(self.mvpp["nnPrRuleNum"]):
            probs = [
                self.nnOutputs[m][t][i * self.n[m] + j]
                for (m, i, t, j) in self.mvpp["nnProb"][ruleIdx]
            ]
            if len(probs) == 1:
                mvppRules += "@{} {}; @{} {}.\n".format(
                    probs[0],
                    self.mvpp["atom"][ruleIdx][0],
                    1 - probs[0],
    ...
    def learn(
        ...
        """
        assert len(dataList) == len(
            obsList
        ), "Error: the length of dataList does not equal to the length of obsList"
        assert (
            alpha >= 0 and alpha <= 1
        ), "Error: the value of alpha should be within [0, 1]"
            ...
            for dataIdx, data in iterator:
                ...
                for m in self.nnOutputs:
                    ...
                    for t in self.nnOutputs[m]:
                        ...
                        nnOutput[m][t] = torch.clamp(
                            nnOutput[m][t], min=10e-8, max=1.0 - 10e-8
                        )
                            ...
                            if isinstance(lossFunc, str):
                                if lossFunc == "cross":
                                    ...
                                    loss = alpha * criterion(
                                        torch.log(nnOutput[m][t].view(-1, self.n[m])),
                                        labelTensor.long().view(-1),
                                    )
                            else:
                                criterion = lossFunc
                                loss = alpha * criterion(
                                    nnOutput[m][t].view(-1, self.n[m]), labelTensor
                                )
                ...
                if alpha < 1:
                    ...
                    for ruleIdx in range(self.mvpp["nnPrRuleNum"]):
                        dmvpp.parameters[ruleIdx] = [
                            self.nnOutputs[m][t][i * self.n[m] + j]
                            for (m, i, t, j) in self.mvpp["nnProb"][ruleIdx]
                        ]
                        if len(dmvpp.parameters[ruleIdx]) == 1:
                            dmvpp.parameters[ruleIdx] = [
                                dmvpp.parameters[ruleIdx][0],
                                1 - dmvpp.parameters[ruleIdx][0],
                            ]
                    ...
                    if storeSM:
                        try:
                            ...
                            if opt:
                                models = dmvpp.find_all_opt_SM_under_obs_WC(
                                    obsList[dataIdx]
                                )
                            else:
                                models = dmvpp.find_k_SM_under_obs(
                                    obsList[dataIdx], k=0
                                )
                        ...
                        if method == "exact":
                            gradients = dmvpp.gradients_one_obs(
                                obsList[dataIdx], opt=opt
                            )
                            ...
                            check = self.satisfy(
                                models[0], self.mvpp["program_asp"] + obsList[dataIdx]
                            )
                            gradients = (
                                dmvpp.mvppLearn(models)
                                if check
                                else -dmvpp.mvppLearn(models)
                            )
                            if check:
                                continue
                        elif method == "penalty":
                            ...
                            models_noSM = [
                                ...
                                for model in models
                                if not self.satisfy(
                                    model, self.mvpp["program_asp"] + obsList[dataIdx]
                                )
                            ]
                            gradients = -dmvpp.mvppLearn(models_noSM)
                        else:
                            print(
                                "Error: the method '%s' should be either 'exact' or 'sampling'",
                                method,
                            )
                    ...
                    for ruleIdx in range(self.mvpp["nnPrRuleNum"]):
                        for probIdx, (m, i, t, j) in enumerate(
                            self.mvpp["nnProb"][ruleIdx]
                        ):
                            self.nnGradients[m][t][i * self.n[m] + j] = (
                                alpha - 1
                            ) * gradientsNN[ruleIdx][probIdx]
                    # Step 2.5: backpropogate
                    for m in nnOutput:
                        for t in nnOutput[m]:
                            if self.device.type == "cuda":
                                nnOutput[m][t].backward(
                                    torch.cuda.FloatTensor(
                                        np.reshape(
                                            np.array(self.nnGradients[m][t]),
                                            nnOutput[m][t].shape,
                                        )
                                    ),
                                    retain_graph=True,
                    ...
                    if self.normalProbs:
                        ...
                        for ruleIdx, ruleGradients in enumerate(gradientsNormal):
                            ...
                            for atomIdx, b in enumerate(dmvpp.learnable[ruleIdxMVPP]):
                                if b == True:
                                    dmvpp.parameters[ruleIdxMVPP][atomIdx] += (
                                        lr * gradientsNormal[ruleIdx][atomIdx]
                                    )
                        dmvpp.normalize_probs()
    ...
    def testNN(self, nn, testLoader):
        ...
        with torch.no_grad():
            for data, target in testLoader:
                ...
                correctionMatrix = (target.int() == pred.int()).view(
                    target.shape[0], -1
                )
        ...
        for dataIdx, data in enumerate(dataList):
            ...
            for model in models:
                if self.satisfy(model, obsList[dataIdx]):
                    correct += 1
                    break
        accuracy = 100.0 * correct / len(dataList)
        return accuracy
    ...
    def testConstraint(self, dataList, obsList, mvppList):
        ...
        for dataIdx, data in enumerate(dataList):
            ...
            for model in self.nnOutputs:
                for t in self.nnOutputs[model]:
                    self.nnOutputs[model][t] = (
                        self.nnMapping[model](data[t].to(self.device)).view(-1).tolist()
                    )
            ...
            for ruleIdx in range(self.mvpp["nnPrRuleNum"]):
                probs = [
                    self.nnOutputs[m][t][i * self.n[model] + j]
                    for (m, i, t, j) in self.mvpp["nnProb"][ruleIdx]
                ]
                if len(probs) == 1:
            ...
            for programIdx, program in enumerate(mvppList):
                ...
                if re.search(r":~.+\.[ \t]*\[.+\]", program) or re.search(
                    r":~.+\.[ \t]*\[.+\]", obsList[dataIdx]
                ):
                    choiceRules = ""
                    for ruleIdx in range(self.mvpp["nnPrRuleNum"]):
                        choiceRules += (
                            "1{" + "; ".join(self.mvpp["atom"][ruleIdx]) + "}1.\n"
                        )
                    mvpp = MVPP(program + choiceRules)
                    models = mvpp.find_all_opt_SM_under_obs_WC(obs=obsList[dataIdx])
                    models = [
                        set(model) for model in models
                    ]  # each model is a set of atoms
        ...
        for programIdx, program in enumerate(mvppList):
            print(
                "The accuracy for constraint {} is {}".format(
                    programIdx + 1, float(count[programIdx]) / len(dataList)
                )
            )
